...
export function createGeneratorNode(scene, x, y, onBuy) {
  const fontFamily = '"Noto Sans SC", "PingFang SC", "Microsoft YaHei", "Hiragino Sans GB", "WenQuanYi Micro Hei", Arial, sans-serif';
  const bg = scene.add.circle(0, 0, 90, 0x102044, 0.95).setStrokeStyle(2, 0x36a2ff, 0.9);
  const text = scene.add.text(0, 0, '', { fontFamily, fontSize: '16px', align: 'center', color: '#d8ecff' }).setOrigin(0.5);
